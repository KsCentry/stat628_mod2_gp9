...
For individuals with body fat < 7%, we use a linear regression model trained on data where body fat ≥ 7% to provide more reasonable estimates.
...
## Three Candidate momdels

Model 1: BODYFAT ~ ABDOMEN + WRIST
Model 2: BODYFAT ~ ABDOMEN + WRIST + AGE
Model 3: BODYFAT ~ ABDOMEN + WRIST + HIP + CHEST

```{r}
model3 = lm(BODYFAT ~ ABDOMEN + WRIST + HIP + CHEST, data = BodyFat_cleaned)
BodyFat_cleaned = subset(BodyFat_cleaned, select = -c(HIP, CHEST, NECK, KNEE, BICEPS))
model1 = lm(BODYFAT ~ ABDOMEN + WRIST, data = BodyFat_cleaned)
summary(model1)
```
...
```{r}
model2 = lm(BODYFAT ~ ABDOMEN + WRIST + AGE, data = BodyFat_cleaned)
summary(model2)
```
...
model_stats = data.frame(
  ...
  MSE = c(mean(residuals(model1)^2), mean(residuals(model2)^2), mean(residuals(model3)^2)),
  RSE = c(summary(model1)$sigma, summary(model2)$sigma, summary(model3)$sigma)
)

rownames(model_stats) = c("Model_1", "Model_2", "Model_3")
print(model_stats)
```

```{r}
max_min <- data.frame(
  R2 = c(0.733, 0.718), Adj_R2 = c(0.73, 0.715), AIC = c(1395, 1375),
  BIC = c(1406, 1400), MSE = c(14.5, 13.8), RSE = c(3.85, 3.72)
)
radar_data <- rbind(max_min[1, ], max_min[2, ], model_stats)

radarchart(radar_data, axistype = 1,
           pcol = c("#66CDAA", "#FF6347", "#8A2BE2"),
           pfcol = c(rgb(102/255, 205/255, 170/255, 0.3),
                     rgb(255/255, 99/255, 71/255, 0.6),
                     rgb(138/255, 43/255, 226/255, 0.2)),
...
legend("right", legend = rownames(radar_data)[3:5],
       col = c("#66CDAA", "#FF6347", "#8A2BE2"),
       pch = 15, 
       bty = "o",
       title = "Models")
...
From this radar chart, we can see that, for AIC, BIC, RSE and MSE, model_2 has small values.
Compared to model_3, which has four variables of circumference measures, the R2 and Adj_R2 values are very close of model_2. So model_2 is the ideal model.


## Some tests of momdel

```{r}
predicted_values = predict(model2, BodyFat_cleaned)
